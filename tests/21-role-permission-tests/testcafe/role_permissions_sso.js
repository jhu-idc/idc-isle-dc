...
test('Ensure SSO login does not re-evaluate roles upon login', async t => {
  ...
  // see that they no roles right now
  ...
  // let's give them global admin privileges
  await t.click(user);
  ...
  // log back in as staff1 an ensure they still have both collection level admin perms
  // and global admin perms
  await t.useRole(staff1AdminSSO);
  await t.navigateTo(pageUserList);
  // let the user check their own perms, since they have global admin this will work.
  user = Selector('div.view-content').find('a').withText('staff1@johnsho…');
  await t.expect(user.count).eql(1);
