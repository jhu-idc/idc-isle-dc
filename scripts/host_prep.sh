...
if [ -f /etc/server-type.conf ]; then
	...
	if [ $servertypename == "test" ]; then
		...
		if [ ! -f /var/idc-isle-dc/codebase/certs/privkey.pem.bak ]; then
			echo "Copying /var/idc-isle-dc/codebase/certs/privkey.pem to /var/idc-isle-dc/codebase/certs/privkey.pem.bak"
		...
		if [ ! -f /var/idc-isle-dc/codebase/certs/cert.pem.bak ]; then
			echo "Copying /var/idc-isle-dc/codebase/certs/cert.pem to /var/idc-isle-dc/codebase/certs/cert.pem.bak"
